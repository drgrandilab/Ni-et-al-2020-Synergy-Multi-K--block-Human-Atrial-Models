...
ax.hist([ Data[1, sel],Data[1, rejected]],20, rwidth=0.8, color=['r', 'k'],histtype='bar', stacked=True)
#ax.hist([Accept_APD_1Hz,Rejected_APD_1Hz ],20,rwidth=0.8, 

ax.plot([317.4-3*43.2, 317.4,317.4+3*43.2], [30, 30, 30], '*')

plt.xlabel('APD$_{90}$ (ms)')
plt.ylabel('Count')
plt.savefig('APD_1Hz_New.pdf', dpi=300)
plt.show()



print (len(Data[1, sel]),len(Data[1, rejected]))
...
# ax.hist([Data2[1, rejected], Data2[1, sel]],20, rwidth=0.9,color=['b', 'k'])
...
plt.xlabel('APD$_{90}$ (ms)')
plt.ylabel('Count')
plt.savefig('f3Hz.pdf', dpi=300)

plt.show()
print np.mean(Data[1, sel]),np.std(Data[1, sel]), 'DATA'
print np.mean(Data2[1, sel]),np.std(Data2[1, sel]), 'Data2'
