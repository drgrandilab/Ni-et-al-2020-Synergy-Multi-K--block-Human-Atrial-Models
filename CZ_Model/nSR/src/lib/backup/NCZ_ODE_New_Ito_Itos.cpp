...
double NCZ_ODE_New_ItoItos(double dt, double Istim, SingleCellPara & het, double *state) {
	...
	Ito = 1.1 * 1.2 * 1.5 * 1.08 * 1.0 * het.Gto * Cm * MAL_gto * itr * (het.Ito_Af * its + (1.0 - het.Ito_Af) * its_s) * (V - Ek);
	...
	// slow component representing Kv1.4
	tau =  800.0 / ( 1.0 + exp((V + 60.0) / 10.0)) + 30.0;   // formuation came from Grandi et al. Ventricular model 2011.
	its_s = inf + (its_s - inf) * exp(-(dt) / tau);
