/* this is a simple implementation of ISO effects in the CRN atrial model
*
*
* the ISO effects are similar to the Grandi et al. 2001 Circ Res Paper.
*
* Haibo Ni <haibo.ni0822@gmail.com>
*
* Tue 05 Apr 2016 12:51:15 BST
*/
...
double CRN_1998_ISO_ODE(const double dt, const double Istim,  SingleCellPara & data, double *state, const double ISO) {
	...
	Km_Na_i = 10.0 ;//* (1.0 - 0.05 * ISO); // millimolar (in sodium_potassium_pump)
	...
	Ca_TRPN = TRPN_max * state[10] / (state[10] + (1-0.25*ISO)*Km_TRPN);
	...
	i_Ca_L = data.GCaL * (1.0 + 0.5 * ISO) * Cm * g_Ca_L * state[4] * state[6] * state[5] * (state[0] - 65.0);
	...
	i_up = I_up_max / (1.0 + K_up*(1-0.5*ISO) / state[10]);
	d_infinity = pow(1.0 + exp((state[0] + 10.0 + ISO*5) / -8.0), -1.0);


	/*ICaL*/
	if (fabs(state[0] + 10.0) < 1.0e-10)
		tau_d = 4.579 / (1.0 + exp((state[0] + 10.0 + ISO*3.0) / -6.24));
	else
		tau_d = (1.0 - exp((state[0] + 10.0 + ISO*3.0) / -6.24)) / (0.035 * (state[0] + 10.0 + ISO*3.0) * (1.0 + exp((state[0] + 10.0 + ISO*3.0) / -6.24)));
	...
	f_infinity = exp(-(state[0] + 28.0 - 5.0*ISO) / 6.9) / (1.0 + exp(-(state[0] + 28.0 - 5.0*ISO) / 6.9));
	tau_f = 9.0 * pow(0.0197 * exp(-pow(0.0337, 2.0) * pow(state[0] + 10.0+ 3.0*ISO, 2.0)) + 0.02, -1.0);
	...
	f_NaK = pow(1.0 + 0.1245 * exp(-0.1 * F * state[0] / (R * T)) + 0.0365 * sigma * exp(-F * state[0] / (R * T)), -1.0);   // voltage dependence.
	...
	i_Kur = (1.0+2.0*ISO)*data.Simple_GKur * Cm * g_Kur * pow(state[19], 3.0) * state[20] * (state[0] - E_K);
	i_Kr = data.GKr * Cm * g_Kr * state[15] * (state[0] - E_K) / (1.0 + exp((state[0] + 15.0) / 22.4));
	i_Ks = (1.0+2.0*ISO)*data.GKs * Cm * g_Ks * pow(state[16], 2.0) * (state[0] - E_K) + phi * Cm * g_Ks * pow(state[16], 2.0) * (state[0] - E_K);
	...
	if (fabs(state[0] - 19.9 + 0 * ISO) < 1.0e-10)
		alpha_xs = 0.00068;
	else
		alpha_xs = 0.00004 * (state[0] - 19.9 + 0 * ISO) / (1.0 - exp((state[0] - 19.9 + 0 * ISO) / -17.0));

	if (fabs(state[0] - 19.9 + 0 * ISO) < 1.0e-10)
		beta_xs = 0.000315;
	else
		beta_xs = 0.000035 * (state[0] - 19.9 + 0 * ISO) / (exp((state[0] - 19.9 + 0 * ISO) / 9.0) - 1.0);

	tau_xs = 0.5 * pow(alpha_xs + beta_xs, -1.0);
	xs_infinity = pow(1.0 + exp((state[0] - 19.9 + /*40.0 **/ ISO) / -12.7), -0.5);
