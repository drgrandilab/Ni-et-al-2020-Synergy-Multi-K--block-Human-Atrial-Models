...
void CRN_1998_ODE_Initialise(double *state, int celltype) {
	...
	state[0]  = -81.18; // V (millivolt) (in membrane)
	state[1 ] = 2.35e-12; // u (dimensionless) (in Ca_release_current_from_JSR_u_gate)
	state[2 ] = 1.0; // v (dimensionless) (in Ca_release_current_from_JSR_v_gate)
	state[3 ] = 0.9992; // w (dimensionless) (in Ca_release_current_from_JSR_w_gate)
	state[4 ] = 1.367e-4; // d (dimensionless) (in L_type_Ca_channel_d_gate)
	state[5 ] = 7.755e-1; // f_Ca (dimensionless) (in L_type_Ca_channel_f_Ca_gate)
	state[6 ] = 9.996e-1; // f (dimensionless) (in L_type_Ca_channel_f_gate)
	state[7 ] = 9.649e-1; // h (dimensionless) (in fast_sodium_current_h_gate)
	state[8 ] = 9.775e-1; // j (dimensionless) (in fast_sodium_current_j_gate)
	state[9 ] = 2.908e-3; // m (dimensionless) (in fast_sodium_current_m_gate)
	state[10] = 1.013e-4; // Ca_i (millimolar) (in intracellular_ion_concentrations)
	state[11] = 1.488; // Ca_rel (millimolar) (in intracellular_ion_concentrations)
	state[12] = 1.488; // Ca_up (millimolar) (in intracellular_ion_concentrations)
	state[13] = 1.39e2; // K_i (millimolar) (in intracellular_ion_concentrations)
	state[14] = 1.117e1; // Na_i (millimolar) (in intracellular_ion_concentrations)
	state[15] = 3.296e-5; // xr (dimensionless) (in rapid_delayed_rectifier_K_current_xr_gate)
	state[16] = 1.869e-2; // xs (dimensionless) (in slow_delayed_rectifier_K_current_xs_gate)
	state[17] = 3.043e-2; // oa (dimensionless) (in transient_outward_K_current_oa_gate)
	state[18] = 9.992e-1; // oi (dimensionless) (in transient_outward_K_current_oi_gate)
	state[19] = 4.966e-3; // ua (dimensionless) (in ultrarapid_delayed_rectifier_K_current_ua_gate)
	state[20] = 9.986e-1; // ui (dimensionless) (in ultrarapid_delayed_rectifier_K_current_ui_gate)
}
