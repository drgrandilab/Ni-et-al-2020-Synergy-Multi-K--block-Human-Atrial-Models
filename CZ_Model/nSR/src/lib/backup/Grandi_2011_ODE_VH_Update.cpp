...
this is the implementation of the updated version of Grandi et al model 2011 by
Voigt - Heijman et al. 2013,

a) Update to IK1 and IKach, to a Nai-dependent manner.
b) Inclusion of ITask-1

1. Voigt N, Heijman J, Trausch A, Mintert-Jancke E, Pott L, Ravens U, et al.
Impaired Na+-dependent regulation of acetylcholine-activated inward-rectifier K+ current
modulates action potential rate dependence in patients with chronic atrial fibrillation.
 Journal of Molecular and Cellular Cardiology. 2013 Aug;61:142–52.


2. Schmidt C, Wiedmann F, Voigt N, Zhou X-B, Heijman J, Lang S, et al.
Upregulation of K2P3.1 K+ Current Causes Action Potential Shortening in
Patients With Chronic Atrial Fibrillation.
Circulation. 2015 Jul 14;132(2):82–92.


Author: Haibo Ni <haibo.ni.0822@gmail.com>

Thu 23 Jun 2016 16:00:32 BST
*/
...
void Grandi_2011_ODE_VH_Update(double t, double *Y, double * dY, void *usr_data)
{
	...
	*/
	...
	GNaB     = 0.597e-3;    // [mS/uF]
	IbarNaK  = 1.26 ;//* (1+0.25);     // [uA/uF]    // 1.8*0.7
	...
	pNaK = 0.01833;// * (1-0.35); // from Schmidt et al. 2015
	gkp  = 0.002;
	...
	GtoSlow = (1-0.6)*0.0156; // 0.0156 (mS/microF)
	GtoFast = (1-0.6)*0.165;   // 0.1144  (mS/microF) Haibo
	...
	/* I_ki */
	/* this is the original code from Grandi et al. 2011 model */
	...
	kiss = aki / (aki + bki);
	I_ki = (1.0 + 1.0 * AF) * 0.0525 * sqrt(Ko / 5.4) * kiss * (Y[38] - ek);*/



	/* I_Ki update by Voigt-Heijman 2013 paper, converted from the original supplementary materal, by Haibo
	Thu 23 Jun 2016 14:17:18 BST
	*/

	ek = 1.0 / FoRT * log(Ko / Y[27]);  // reversal potential

	aki = 1.0 / (1.0 + pow(Y[30] / 8.247, 1.388)) * 1.0 / (1.0 + exp(0.2385 * (Y[38] - ek - 59.215)));
	bki = (0.49124 * exp(0.08032 * (Y[38] + 5.476 - ek)) + exp(0.06175 * (Y[38] - ek - 594.31))) / (1.0 + exp(-0.5143 * (Y[38] - ek + 4.753)));
	kiss = aki / (aki + bki);
	I_ki = (1+0.45)*(1.0 + 0.8 * AF) * 0.0525 * sqrt(Ko / 5.4) * kiss * (Y[38] - ek);



	/* inclusion of IKach from Voigt-Heijman  et al. 2013*/

	double f_kach_Na = 1.6863 / (1 + 1000.0 / Y[30] / Y[30] / Y[30]);
	double R_kach = 0.055 + 0.40 / ( 1 + exp((Y[38] - ek + 9.53) / 17.18));
	double Gkach = 0.1275 * (1 + f_kach_Na);
	if (AF) {
		Gkach = 0.0691; // from the supplementary material of Voigt-Heijman et al. 2013
	}

	double CCh_Conc = 0.00001;
	double I_kach = Gkach * sqrt(Ko / 5.4) * (CCh_Conc / (CCh_Conc + 0.125)) * R_kach * (Y[38] - ek);


	/* INa to be replaced by the CRN-INa model */



	/* ITask-1 Channel model */


	double GK2p = 0.005;  // nS/pF
	double K2p_inf = 0.2 + 0.8 / (1.0 + exp((Y[38] - 10) / -14.0));
	double tau_K2p = 0.2 + 40.0 / (1.0 + exp((Y[38] + 25) * (Y[38] + 25) / 80.0));

	if (AF) {
		K2p_inf = 0.2 + 0.8 / (1.0 + exp((Y[38] + 5) / -14.0));
		GK2p = 0.0145; // 2.9 times
		// tau_K2p = 0.2 + 40.0 / (1.0 + exp((Y[38] + 25) * (Y[38] + 25) / 80.0));
	}

	dY[41] = (K2p_inf - Y[41]) / tau_K2p;
	double I_K2p = GK2p * Y[40] * (Y[38] - ek);


	kp_kp = 1.0 / (1.0 + exp(7.488 - Y[38] / 5.98));
	I_kp_junc = Fjunc * gkp * kp_kp * (Y[38] - ek);
	I_kp_sl = Fsl * gkp * kp_kp * (Y[38] - ek);
	...
	*/
	...
	// CRN Atrial Model INa
	if (Y[38] < -40.0)
		ah = 0.135 * exp((Y[38] + 80.0) / -6.8);
	else
		ah = 0.0;

	if (Y[38] < -40.0)
		bh = 3.56 * exp(0.079 * Y[38]) + 3.1e5 * exp(0.35 * Y[38]);
	else
		bh = 1.0 / (0.13 * (1.0 + exp((Y[38] + 10.66) / -11.1)));
	...
	if (Y[38] < -40.0)
		aj = (-1.2714e5 * exp(0.2444 * Y[38]) - 3.474e-5 * exp(-0.04391 * Y[38])) * (Y[38] + 37.78) / (1.0 + exp(0.311 * (Y[38] + 79.23)));
	else
		aj = 0.0;

	if (Y[38] < -40.0)
		bj = 0.1212 * exp(-0.01052 * Y[38]) / (1.0 + exp(-0.1378 * (Y[38] + 40.14)));
	else
		bj = 0.3 * exp(-2.535e-7 * Y[38]) / (1.0 + exp(-0.1 * (Y[38] + 32.0)));
	...
	if (Y[38] == -47.13)
		...
		alpha_m = 0.32 * (Y[38] + 47.13) / (1.0 - exp(-0.1 * (Y[38] + 47.13)));

	beta_m = 0.08 * exp(-Y[38] / 11.0);
	mss = alpha_m / (alpha_m + beta_m);
	taum = 1.0 / (alpha_m + beta_m);
	GNa *= 0.3;
	...
	/*//% Late I_Na
	GNaL=0.0025*AF;
	aml = 0.32 * (Y[38] + 47.13) / (1.0 - exp(-0.1 * (Y[38] + 47.13)));
	bml = 0.08 * exp(-Y[38]) / 11.0);
	hlinf = 1.0 / (1.0 + exp((Y[38] + 91.0) / 6.1));
	tauhl = 600.0;
	...
	*/
	...
	I_tos = 0.0 * GtoSlow * Y[20] * Y[22] * (Y[38] - ek); // Haibo I_to slow has been removed
	...
	// 	ac_Ikur_inf = 1.0 / (1.0 + exp(-(Y[38] + mal_IKur_ac_shift + 6.0) / (8.6 * mal_IKur_ac_grad)));
	// 	inac_Ikur_inf = 1.0 / (1.0 + exp((Y[38] + mal_IKur_inac_shift + 7.5) / (10.0 * mal_IKur_inac_grad)));
	// 	dY[39] = (ac_Ikur_inf - Y[39]) / tau_ac_Ikur;
	// 	dY[40] = (inac_Ikur_inf - Y[40]) / tau_inac_Ikur;
	// 	Ikur = mal_mult * Gkur * Y[39] * Y[40] * (Y[38] - ek);
	// 	// Ikur = Gkur * Y[39] * Y[40] * (Y[38] - ek);
	// }
	...
	tau_inac_Ikur = 590.0 / (1.0 +  exp((Y[38] + 60.0) / 10.0)) + 3050.0;
	// shift and change slope according to the effects of the mutations, by haibo
	ac_Ikur_inf = 1.0 / (1.0 + exp(-(Y[38] + Data->Simple_Ikur_ac_shift + 6.0) / (8.6 * Data->Simple_Ikur_ac_grad)));
	inac_Ikur_inf = 1.0 / (1.0 + exp((Y[38] + Data->Simple_Ikur_inac_shift + 7.5) / (10.0 * Data->Simple_Ikur_inac_grad)));
	dY[39] = (ac_Ikur_inf - Y[39]) / tau_ac_Ikur;
	dY[40] = (inac_Ikur_inf - Y[40]) / tau_inac_Ikur;
	Ikur = Data->Simple_GKur * Gkur * Y[39] * Y[40] * (Y[38] - ek);


	I_K_tot = I_to + I_kr + I_ks + I_ki - 2.0 * I_nak + I_CaK + I_kp + Ikur  + I_kach + I_K2p;
	dY[27] = -I_K_tot * Cmem / ((Vsl + Vmyo + Vjunc) * Frdy);
