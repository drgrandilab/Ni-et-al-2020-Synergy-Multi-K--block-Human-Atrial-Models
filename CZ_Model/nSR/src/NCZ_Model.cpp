...
int main(int argc, char const *argv[])
{
	...
	// Total_time = 50 * BCL;
	// double S2 = 0.5 * BCL;
	Total_time = 310000.0;//(S1_Num - 1) * BCL + 2 * BCL;
	...
	if (Mode == "SimDrug")   // read the ICs.bin, set the time to be 60 s before termination of simulation
	{
		...
		if (infile.is_open())
		{
			infile.read(reinterpret_cast<char*>(state), sizeof(double)*STATENUM);
			infile.close();
			Time_Start = Total_time - 200 * 1000 - time_step;
		} else {
			std::cout << "ICs file not opened..." << std::endl;
		}
	}
	...
	for (t = Time_Start; t < Total_time; t = t + time_step)
	{
		...
		if (Mode == "SimDrug")   // read the ICs.bin, set the time to be 60 s before termination of simulation
		{
			if (t > Total_time - 200 * 1000) {     // Applying the drug here
				// cell_para.drug_INa_concen = 60.0 * 1e-6; // apply 60 umol INa blocker.. or 60.0*1e-6 mol;

				// cell_para.GKur  *= (1 - atof(argv[26])); //Drug block model
		...
		double dv = NCZ_ODE_Update(time_step, stim, cell_para, state);
		...
		// only output once
		if (t > Total_time - 100 * 1000 - 0.5 * time_step and t < Total_time - 100 * 1000 + 0.5 * time_step) {
			if (Mode == "Normal")
			{
