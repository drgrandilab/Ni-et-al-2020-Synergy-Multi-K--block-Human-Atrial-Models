%$// 12:14:29, Tue, 06-November-2018, By Haibo
% to speed up the model

function output = ham_ina_isk_drug_model(t,y,param,runType)
%
% global timeVC p.emVC_1 p.emVC_2  p.EAD_Vm p.EAD_t
% global p.altTime p.altCaj p.altCasl
% global stim stim_t

% [prot_par, drug_par, exp_par, ikur_par, AF] = p.option; %17
% [ptimeVC p.emVC_1 p.emVC_2 p.EAD_Vm p.EAD_t p.altTime p.altCaj p.altCasls] = p.data;
sfactor = param.ScaleFactors;


Multiple_AF_conditions = 1;
...
S_GKAch = 1;  % not considered here for sensitivity analysis
...
ISK_flag = 0; % with 0 ISK is blocked
pcask_shift = 0; % shift of Ca-sens (default: 0) kd = 10^(pcask_shift+3-6.3) % (mM)
...
elseif protocol_index == 3, protocol = 'tri_ap_clamp';
...
elseif protocol_index == 13, protocp.EAD_tol = 'pace_cc_ead';
...
% Drug concentrations
%drug_index = 0; drug = 0;               % Drug Free
%drug_index = 1; drug = 100 * (1E-6);     % Ranolazine (M)
%drug_index = 2; drug = 100 * (1E-6);    % Lidocaine (M)
%drug_index = 3; drug = 10 * (1E-6);     % Flecainide (M)
%drug_index = 4; drug = 0.3 * (1E-6);      % GS967 (M)
...
if (AF == 1)
    ...
    if (Multiple_AF_conditions == 1 )
        S_GKur_AF = param.S_GKur_AF;
        S_GSK_AF  = param.S_GSK_AF;
        S_GK2P_AF = param.S_GK2P_AF;
    end
end
...
GNa_hh = S_GNa * 23*(1-0.1*AF);  % [mS/uF]
GNa = S_GNa * 10*(1-0.1*AF);%*(1+0.2*EAD);  % [mS/uF]
...
ina_hh_flag=1; % 1 original, 2 Jussi
if flagMina == 0
    if ina_hh_flag == 1, % original
    ...
    if ina_hh_flag == 2, % Jussi
        mss = 1 / (1 + exp( -(47.1 + y(39)) / 7.4 ));
        taum = 1000 * (0.00013 * exp(-((y(39)+48)/15)^2) + 0.000045 / (1 + exp(-(y(39)+42)/5)) + 0.00001);
        hss = 1 / (1 + exp((y(39) + 74.4) / 6 ));
        tauh = 1000 * (0.0002 + 0.11/(1 + exp((y(39)+58)/6) + exp(-(y(39)+58)/15)));
        jss = hss;
        tauj = 1000 * (0.003 + 0.60/(1 + exp((y(39)+58)/6) + exp(-(y(39)+58)/15)));
    end
...
dCNa2  =  betaNa8*LCNa2+alphaNa1*CNa3+betaNa2*CNa1+alphaNa5*ICNa2-(coeff_CNa2)*CNa2;
dCNa1  = betaNa8*LCNa1+alphaNa2*CNa2+betaNa3*ONa+alphaNa5*IFNa-(coeff_CNa1)*CNa1;
dONa   = betaNa8*LONa+alphaNa3*CNa1+betaNa4*IFNa-(coeff_ONa)*ONa;
dIFNa  = alphaNa4*ONa+betaNa5*CNa1+betaNa6*I1Na+alphaNa2*ICNa2-(coeff_IFNa)*IFNa;
dI1Na  = alphaNa6*IFNa+betaNa7*I2Na-(coeff_I1Na)*I1Na;
dCNa3  = betaNa8*LCNa3+betaNa1*CNa2+alphaNa5*ICNa3-(coeff_CNa3)*CNa3;
dICNa2 = alphaNa1*ICNa3+betaNa2*IFNa+betaNa5*CNa2-(coeff_ICNa2)*ICNa2;
dICNa3 = betaNa1*ICNa2+betaNa5*CNa3-(coeff_ICNa3)*ICNa3;
dLONa  = alphaNa3*LCNa1+alphaNa8*ONa-(coeff_LONa)*LONa;
dLCNa1 = alphaNa8*CNa1+alphaNa2*LCNa2+betaNa3*LONa-(coeff_LCNa1)*LCNa1;
dLCNa2 = betaNa2*LCNa1+alphaNa8*CNa2+alphaNa1*LCNa3-(coeff_LCNa2)*LCNa2;
dLCNa3 = alphaNa8*CNa3+betaNa1*LCNa2-(coeff_LCNa3)*LCNa3;
...
% ydot(75:87) = 0;%%[dCNa3_c dCNa2_c dCNa1_c dONa_c dLCNa3_c dLCNa2_c dLCNa1_c dLONa_c dICNa3_c dICNa2_c dIFNa_c dI1Na_c dI2Na_c];
% ydot(88:100) = 0;%%[dCNa3_n dCNa2_n dCNa1_n dONa_n dLCNa3_n dLCNa2_n dLCNa1_n dLONa_n dICNa3_n dICNa2_n dIFNa_n dI1Na_n dI2Na_n];

I_Na_junc2 = Fjunc*GNa*(ONa+LONa)*(y(39)-ena_junc); % +(y(39)>-82.5)*0.2e-6
I_Na_sl2 = Fsl*GNa*(ONa+LONa)*(y(39)-ena_sl); % +(y(39)>-82.5)*0.2e-6
...
if IKur_M_flag == 0
    ...
    gkur_hh = 0.85*S_GKur * S_GKur_AF*(1+2*ISO)* 0.045*(1+0.2*RA);   %after incorporation of ISK and IK2P, IKur reduced by 15% first.
    ...
    gkur_m = S_GKur * (1-Complete_Block)*(1.0-0.5*IKur_ur_50)*S_GKur_AF*(1+2*ISO)*0.045*1.36*(1+0.2*RA); %nS/pF maleckar 0.045*1.36 based on Feng physiological data (Figure 4)
    % gkur_m = 0.9*gkur_hh;
...
%% ITask1-1 to be included

%% ISK to be updated with the Kuvimarki one
%% I_sk: small-conductance Ca-activated K current
%% this is Stefano's model, not used here.
% gsk = 0.025;
% kdsk = (10^(pcask_shift-3.3));
% gsk_junc = 1*(1/(1+ exp((log10(kdsk)-log10(y(36)))/0.3)));
% gsk_sl = 1*(1/(1+ exp((log10(kdsk)-log10(y(37)))/0.3)));

% % v6 (Dobrev 2nd)
% skss = 1/(1+exp(0.075*(y(39)-ek-20))) + 0.50/(1+exp(0.03*(-(y(39)-ek-90))));
% gsk_max = ISK_flag*gsk; % v6 - nSR
% if AF==1, % v6 - cAF
%     skss = 1/(1+exp(0.075*(y(39)-ek-15))) + 0.55/(1+exp(0.03*(-(y(39)-ek-60))));
%     gsk_max = ISK_flag*gsk/0.025*0.110;
% end
% % pAF: skss = 1/(1+exp(0.075*(y(39)-ek-10))) + 0.65/(1+exp(0.03*(-(y(39)-ek-100)))); gsk_max = ISK_flag*0.060;

% I_sk_junc = Fjunc*gsk_max*gsk_junc*skss*(y(39)-ek);
% I_sk_sl = Fsl*gsk_max*gsk_sl*skss*(y(39)-ek);
...
% add my version of IK2p and Itask

% Adding Task-1/K2P channel

GK2p = 0.005;

o_k2p = y(86+1);

IK2p = S_GK2P_AF * S_GK2P * GK2p * o_k2p * (y(39) - ek);
...
ydot(86+1) = (inf_O_K2p - y(86+1)) / tau;



KCa_on = 47e6 / 1000.0; % change from s to ms.
KCa_off = 13 / 1000.0;   % change from s to ms.
O_SK_j = y(86+2);
O_SK_sl = y(86+3);
% GSK = 0.06 * 3.6;
GSK = 0.025*3;

I_sk_junc =  S_GSK_AF * Fjunc*S_GSK* GSK * O_SK_j * (1.0 / (1.0 + exp( (y(39) - ek + 120) / 45.0))) * (y(39) - ek);
I_sk_sl =  S_GSK_AF * Fsl*S_GSK* GSK * O_SK_sl * (1.0 / (1.0 + exp( (y(39) - ek + 120) / 45.0))) * (y(39) - ek);
I_sk = I_sk_junc+I_sk_sl;

Caj = y(36);
ydot(86+2) = (1 - O_SK_j) * KCa_on * Caj * Caj - KCa_off * O_SK_j;

Casl = y(37);
ydot(86+3) = (1 - O_SK_sl) * KCa_on * Casl * Casl - KCa_off * O_SK_sl;
...
J_SRleak = (1.0+0.25*AF)*5.348e-6*(y(31)-y(36));           %   [mM/ms]
    ...
    switch lower(protocol)
        ...
        case 'pace_cc_erp', % pace ERP protocol
            ...
            elseif t > rec_interval && t <= rec_interval+5
                %             I_app = 12.5*1;
                %             I_app = 2*(12.5*0.17); % 2*DTE - ISK, Stefano's
                %             I_app = 2*(12.5*0.15); % 2*DTE - IKur Proj 1 Hz
                %             I_app = 2*(12.5*0.32); % 2*DTE - IKur Proj 3 Hz
                %             I_app = 2*(12.5*0.28); % 2*DTE - IKur Proj 3 Hz, used
                
                % DTE
                I_app = 2*(12.5*0.2); % 2*DTE - IKur Proj 1 Hz and 3 Hz
                %             I_app = (12.5*p(17)); % 2*DTE - IKur Proj 1 Hz and 3 Hz
...
elseif (nargin == 4) & strcmp(runType,'currents')
    ...
    %     currents=[I_Na I_KAch I_Catot I_ncx vmax I_sk I_ki I_kur];
    currents = [I_Na I_KAch I_Catot I_ncx vmax I_sk I_ki I_kur I_kur_hh I_kur_m, I_sk, IK2p];
    %currents=[alphaNa1 betaNa1 alphaNa2 betaNa2 alphaNa3 betaNa3 alphaNa4 betaNa4 alphaNa5 betaNa5 alphaNa6 betaNa6 alphaNa7 betaNa7 alphaNa8 betaNa8];
    %currents=[I_Na I_KAch I_Catot I_ncx vmax I_sk I_ki I_to I_ks I_kr I_kur I_nak];
