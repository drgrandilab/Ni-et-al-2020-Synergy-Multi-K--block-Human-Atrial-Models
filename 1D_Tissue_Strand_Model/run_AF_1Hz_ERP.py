...
def run_ERP(Para, AF_model, block_para,f,Ini_cond_File, S2, ID=0):
	# global mut
	#call(["./main","BCL", str(S1), "S2", str(S2), "Mutation", mut, "S1_number", "20", "ISO", ISO])  #
	# call(['./SAN', str(P1), str(P2)]) #"Mutation", mut, "S1_number", "20", "ISO", ISO])  #
	# call(['./NCZ_Model_New_Ito', '250', '14', '250', 'WT', 'Normal', '0', '0', str(P1), str(P2), str(P3),str(P4),str(P5),str(P6)], stdout=f)
	# call(['./NCZ_Model_New_Ito', '500', '14', '500', 'WT', 'Normal', '0', '0', str(P1), str(P2), str(P3),str(P4),str(P5),str(P6)], stdout=f)
	P=['./ONE_D_NCZ_VEC_ERP', 'BCL', str(BCL), 'S2', str(S2), 'S1_number', str(100+int(1010000/BCL)-1), 'AF', '4', 'Popl_SF_Number', '15', 'Initial_conditions', Ini_cond_File, 'Time_Start', str( 1010000+100*BCL -2*BCL - 10 ), 'Total_time', str(1010000+100*BCL) ]
	P.append('Popul_scaling_factors')
	for i in range(len(Para)):
		P.append(str(Para[i]))
...
def get_upper_bound(para_set, AF_set, block_set,OneD_files,Cell_ID, S2):
	...
	go = check_if_bilateral_cond(run_ERP(para_set, AF_set, block_set,OneD_files, 'Specific', S2, Cell_ID ))
	lastgo = go
	# go = reduce_S2(run_ERP(para_set, AF_set, block_set,OneD_files, 'Specific', S2, Cell_ID ))

	# run_ERP(Para, AF_model, block_para,f,Ini_cond_File, S2, ID=0)
...
def get_lowwer_bound(para_set, AF_set, block_set,OneD_files,Cell_ID, S2):
	...
	go = check_if_bilateral_block(run_ERP(para_set, AF_set, block_set,OneD_files, 'Specific', S2, Cell_ID ))
	lastgo = go
	# go = reduce_S2(run_ERP(para_set, AF_set, block_set,OneD_files, 'Specific', S2, Cell_ID ))
...
ERP_results=[]
for i in range(8):
	ERP_results.append(open("ERP.results..AF_Model." + sys.argv[1] + '.block.'+str(i), "w+"))
...
# Drug_ID = 0;
# run_ERP(para_set[Cell_ID], AF_set[AF_ID], block_set[Drug_ID],OneD_files[Drug_ID], 'Specific', S2, Cell_ID )
...
for ii  in range(len(selected_para)):
	...
	for Drug_ID in range(8):
		...
		print >> ERP_results[Drug_ID], Cell_ID, S2_upper,d_S2_up
		# S2=131.98359375 +0.2
		...
		# go = reduce_S2(run_ERP(para_set[Cell_ID], AF_set[AF_ID], block_set[Drug_ID],OneD_files[Drug_ID], 'Specific', S2, Cell_ID ))
		# lastgo = go
		# go = reduce_S2(run_ERP(para_set[Cell_ID], AF_set[AF_ID], block_set[Drug_ID],OneD_files[Drug_ID], 'Specific', S2, Cell_ID ))
		...
		# 			S2 =S2+d_S2+0.5
		# 		lastgo=go
		# 		go=check_if_bilateral_cond(run_ERP(para_set[Cell_ID], AF_set[AF_ID], block_set[Drug_ID],OneD_files[Drug_ID], 'Specific', S2, Cell_ID ))
		# 		print  >>  f, "S1= ", BCL, "S2= ", S2, "d_S2 = ", d_S2, "go = ", go, ' Cell_ID = ', Cell_ID
