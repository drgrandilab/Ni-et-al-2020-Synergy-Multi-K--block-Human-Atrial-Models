...
double NCZ_ODE_Update(double dt, double Istim, SingleCellPara & het, double *state) {
	...
	double o_k2p = state[37];
	double O_SK = state[38];
	double h_NaL = state[39];
	...
	INaL = het.GNaL * 0.001 * Cm * CRN_gna * m * m * m * h_NaL * (V - Ena);  // GNaL = 0.1% of GNa; // 10:38:23, Sat, 11-November-2017, By Haibo

	INa += INaL;

	// IKr
	// IKr = 1.25 * het.GKr * Cm * CRN_gkr * xr * (V - Ek) / (1 + exp((V + 15) / 22.4)); //
	IKr = 0.85 * het.GKr * Cm * CRN_gkr * xr * (V - Ek) / (1 + exp((V + 15) / 22.4)); // 90% haibo
	...
	IKs = 0.85 * het.GKs * Cm * CRN_gks * xs * xs * (V - Ek); // 1.1 times larger. haibo
	...
	// tau = 0.5*8.91 * (49.008 / (1 + exp((V + 2.04 + 28.1273 - 1) / (10.63680 * 0.42)))) * (0.867157776373 / (1 + exp((V + 0.6476 + -2+64) / (-4.69634 * 0.82)))) + 60;  // 11:30:03, Wed, 29-November-2017, By Haibo  new tau for ICaL
	...
	IKur = 1.1*1.05 * 1.0 / 0.9 * het.GKur * Cm * CNZ_gkur * (4.5128 + 1.899769 / (1.0 + exp((V - 20.5232) / (-8.26597)))) * (1 - IKur_BO - IKur_BC) * CNZ_a * (0.65 * CNZ_i + 0.35 * CNZ_is) * (V - Ek);
	...
	//IK1
	IK1 =  0.9 * het.GK1 * Cm * CRN_gk1 * (V - Ek + het.IK1_v_shift) / (1 + exp(0.07 * (V - (Ek + 6.94) + het.IK1_v_shift)));
	...
	IK2p = het.GK2P * Cm * GK2p * o_k2p * (V - Ek);
	...
	ISK =  het.GSK*Cm * GSK * O_SK * (1.0 / (1.0 + exp( (V - Ek + 120) / 45.0))) * (V - Ek);
	...
	IbCa = het.GbCa*1.0 * Cm * CRN_gbca * (V - Eca); // increased background IbCa, haibo
	// IbNa  1.7
	IbNa = het.GbNa*1.25 * 1.0 * Cm * CRN_gbna * (V - Ena); // increase. haibo

	//ICap  // 1.26 // 1.15
	ICap = het.GCap * 0.9 * 1.0 * Cm * CRN_icapbar * Cass / (Cass + CRN_kmcap); // change, haibo
	// ICap = 1.1 * het.GCap * Cm * CRN_icapbar * Cass / (Cass + CRN_kmcap);  // change, haibo



	//INaCa, times 1.4, Haibo
	INaCa = het.GNaCa * 1.1 * Cm * CRN_knacalr / (pow(CRN_kmnalr, 3.0) + pow(CRN_nac, 3.0)) / (CRN_kmcalr + CRN_cac) /
	...
	INaK = het.GNaK *1.0 * Cm * CRN_inakbar * fnak * CRN_kc / (CRN_kc + CRN_kmko) / (1 + pow(/*CRN_kmnai*/8 / nai, 4)); // increase, Haibo.
	...
	return (Istim + (INa + Ito + IKur + IKr + IKs + ICaL + IK1 + IK2p + ISK + IbK + IbNa + IbCa + INaCa + INaK + ICap + If + ICaT ) / Cm);
} // end CNZ
...
void NCZ_ODE_Update_Initialise(double *state, int celltype) {
	...
	state[37] = 0.2;   // 0.9517278864;
	state[38] = 0.0;   // 0.9517278864;
	state[39] = 0.2;   // 0.9517278864;
}
