...
def get_data(model):
	data = np.loadtxt('ERP.results..AF_Model.1.block.%d'%model)
...
for i in range(8):
	...
	data_pd.columns = [ 'SIM_ID', 'ERP', 'upper_d_S2']
	data_pd.index = (data_pd['SIM_ID'])
	print data_pd.index.values
	print data_pd.columns
	# data_pd.to_pickle('vw.results..AF_Model.5.block.%d.pkl'%i)

	data_apd.append(data_pd['ERP'])
	# data_lower.append(data_pd['lower'])


res = pd.concat (data_apd,axis=1, keys=['Baseline', 'IK2p', 'ISK', 'IK2p_ISK', 'IKur', 'IK2p_IKur', 'ISK_IKur','IK2P_ISK_IKur'])
res.to_pickle('Summarise_ERP_Model_1.pkl')
# res.to_csv('Summarise_vw_Model_5.csv')
# res_lower = pd.concat (data_lower,axis=1, keys=['Baseline', 'IK2p', 'ISK', 'IK2p_ISK', 'IKur', 'IK2p_IKur', 'ISK_IKur','IK2P_ISK_IKur'])
# res_lower.to_pickle('Summarise_vw_lower_Model_5.pkl')
